...
function generateBracket() {
  ...
  bk = {
    ...
    gf: mkM('GF', null, null, false),
    tp: mkM('TP', null, null, false),
    ranking: {},
    _paths: {},
  };
  ...
  canvas.id = 'bkCanvas'; canvas.className = 'bk-canvas';
  const svg = document.createElementNS('http://www.w3.org/2000/svg','svg');
  svg.id = 'bkSvg'; svg.className = 'bk-svg';
...
function buildHalf(players, prefix) {
  ...
  while (prev > 1) {
    ...
    rounds.push(Array.from({length:cnt}, (_,i) => mkM(`${prefix}-${rounds.length}-${i}`, null, null, false)));
    prev = cnt;
  }
  return rounds;
}
function mkM(id, p1, p2, bye) {
  return { id, p1, p2, winner:null, bye, _el:{}, _paths:{} };
}
...
function renderBracketDOM(canvas, svgEl) {
  ...
  bk.left.forEach((round, ri) => {
    round.forEach((match, mi) => {
      placeMatch(canvas, svgEl, match, lRX(ri), lSY+lC[ri][mi], 'left', ri, mi);
    });
    if (ri < bk.left.length-1) {
      round.forEach((match, mi) => {
        const nm = bk.left[ri+1][Math.floor(mi/2)];
        const tk = mi%2===0?'p1':'p2';
  ...
  bk.right.forEach((round, ri) => {
    round.forEach((match, mi) => {
      placeMatch(canvas, svgEl, match, rRX(ri), rSY+rC[ri][mi], 'right', ri, mi);
    });
    if (ri < bk.right.length-1) {
      round.forEach((match, mi) => {
        const nm = bk.right[ri+1][Math.floor(mi/2)];
        const tk = mi%2===0?'p1':'p2';
...
function calcCenters(rounds) {
  ...
  for (let ri=1; ri<rounds.length; ri++) {
    ...
    centers.push(rounds[ri].map((_,mi) => {
      const a = prev[mi*2]    ?? prev[prev.length-1];
      const b = prev[mi*2+1] ?? prev[mi*2];
      return (a+b)/2;
    }));
  }
  return centers;
...
function placeMatch(container, svg, match, x, cy, side, ri, mi) {
  const y1 = cy - BK_H - BK_VS/2;
  const y2 = cy + BK_VS/2;

  const el1 = makeSlot(match, 'p1');
  el1.style.left = x+'px'; el1.style.top = y1+'px';
  container.appendChild(el1); match._el.p1 = el1;

  // vs 虛線
  const vl = mkSvg('line');
  vl.setAttribute('x1',x+10); vl.setAttribute('x2',x+BK_W-10);
  vl.setAttribute('y1',cy);   vl.setAttribute('y2',cy);
  vl.setAttribute('stroke','rgba(201,168,76,0.22)');
  vl.setAttribute('stroke-width','1.5');
  vl.setAttribute('stroke-dasharray','3 3');
  svg.appendChild(vl);

  if (!match.bye) {
    const el2 = makeSlot(match, 'p2');
    el2.style.left = x+'px'; el2.style.top = y2+'px';
    container.appendChild(el2); match._el.p2 = el2;
  }

  if (ri===0 && match.p1 && match.p2 && !match.bye) {
    enableClick(match, side, ri, mi);
  }
}

function makeSlot(match, key) {
  const el = document.createElement('div');
  el.className = 'bk-slot dragon-border';
  ...
  el.innerHTML = `<span class="dragon-bl"></span><span class="dragon-br"></span>
    <span class="bk-win-bar"></span>
    <span class="bk-slot-name${!player&&!match.bye?' bk-placeholder':''}">${
      player||(match.bye?'輪空':'待定')}</span>`;
  return el;
}
...
function pickWinner(match, winKey, side, ri, mi) {
  ...
  match.winner  = match[winKey];
  match._el[winKey]?.classList.add('bk-slot--winner');
  match._el[winKey]?.classList.remove('bk-slot--clickable');
  match._el[loseKey]?.classList.add('bk-slot--loser');
  match._el[loseKey]?.classList.remove('bk-slot--clickable');
  if (match._el.p1) match._el.p1.onclick = null;
  if (match._el.p2) match._el.p2.onclick = null;

  const rounds    = bk[side];
  const nextRi    = ri+1;
  const nextMi    = Math.floor(mi/2);
  const toKey     = mi%2===0?'p1':'p2';
  ...
  animateFlow(pathEl, () => {
    if (nextMatch) {
      nextMatch[toKey] = match.winner;
      fillSlot(nextMatch._el[toKey], match.winner);
      if (nextMatch.p1 && nextMatch.p2 && !nextMatch.winner) {
        enableClick(nextMatch, side, nextRi, nextMi);
      }
      checkSemi(side, nextRi, nextMi, nextMatch);
    } else {
      checkSemi(side, ri, mi, match);
    }
  });
}
...
function checkSemi(side, ri, mi, match) {
  const rounds = bk[side];
  if (ri !== rounds.length-1 || !match.winner) return;
  const winner = match.winner;
  const loser  = match.p1===winner ? match.p2 : match.p1;
  const gfKey  = side==='left'?'p1':'p2';

  bk.gf[gfKey] = winner;
  setTimeout(() => { fillSlot(bk.gf._el[gfKey], winner); flashSlot(bk.gf._el[gfKey]); }, 200);
  if (loser) {
    bk.tp[gfKey] = loser;
    setTimeout(() => { fillSlot(bk.tp._el[gfKey], loser); flashSlot(bk.tp._el[gfKey]); }, 350);
  }
...
function placeFinals(container, svg, cx, cy) {
  ...
  const mkFS = (match, key, placeholder, x, y) => {
    const el = document.createElement('div');
    el.className = 'bk-slot dragon-border bk-final-slot';
    ...
    el.dataset.matchId=match.id; el.dataset.slot=key;
    match._el[key]=el; container.appendChild(el);
  };
  const mkLbl = (t,x,y,sz='0.88rem') => {
    const el=document.createElement('div');
    el.style.cssText=`position:absolute;left:${x}px;top:${y}px;width:${fw}px;text-align:center;font-family:Rajdhani,sans-serif;font-size:${sz};font-weight:700;color:var(--gold-dk);`;
    el.textContent=t; container.appendChild(el);
  };
  const mkVs=(x,y)=>{
    const el=document.createElement('div');
    el.style.cssText=`position:absolute;left:${x}px;top:${y}px;width:${fw}px;height:${BK_VS}px;display:flex;align-items:center;justify-content:center;font-family:Rajdhani,sans-serif;font-size:.72rem;font-weight:700;color:var(--text2);letter-spacing:2px;`;
    el.textContent='VS'; container.appendChild(el);
  };

  mkLbl('🏆 冠軍賽', fx, cy-22);
  mkFS(bk.gf,'p1','待左半冠軍',fx,cy);
  mkVs(fx,cy+BK_H);
  mkFS(bk.gf,'p2','待右半冠軍',fx,cy+BK_H+BK_VS);

  const tpB=cy+matchH+36;
  mkLbl('🥉 季軍賽',fx,tpB-20,'0.78rem');
  mkFS(bk.tp,'p1','待左半落敗',fx,tpB);
  mkVs(fx,tpB+BK_H);
  mkFS(bk.tp,'p2','待右半落敗',fx,tpB+BK_H+BK_VS);

  const rb=document.createElement('div');
  rb.id='bkRankBoard'; rb.className='bk-rank-board';
  rb.style.cssText=`position:absolute;left:${fx-5}px;top:${tpB+BK_H*2+BK_VS+16}px;width:${fw+10}px;display:none;`;
...
function drawConn(svg, fromMatch, toMatch, toKey, x1, y1, x2, y2) {
  ...
  path.classList.add('bk-conn');
  svg.appendChild(path);
  const key=`${fromMatch.id}→${toMatch.id}-${toKey}`;
  bk._paths[key]=path; fromMatch._paths[key]=path;
}
...
function animateFlow(pathEl, callback) {
  ...
  function step(ts){
    if(!start)start=ts;
    const p=Math.min((ts-start)/550,1), pT=Math.max(p-0.12,0);
    ...
    else { dot.remove(); tail.remove(); callback?.(); }
  }
...
function fillSlot(el, name) {
  if (!el) return;
  const ne=el.querySelector('.bk-slot-name'); if(!ne) return;
...
function flashSlot(el) {
  if(!el) return;
  el.classList.add('bk-slot--flash');
  setTimeout(()=>el.classList.remove('bk-slot--flash'),450);
}
function autoAdvanceByes() {
  ['left','right'].forEach(side=>{
    bk[side][0].forEach((match,mi)=>{
      if(match.bye&&match.p1&&!match.winner)
        setTimeout(()=>pickWinner(match,'p1',side,0,mi),80+mi*60);
    });
  });
}
function updateRankBoard() {
  ...
  el.style.display='';
  el.innerHTML=`<div class="bk-rank-title">🎖️ 目前排名</div>`+
    medals.filter(m=>r[m.k]).map(m=>`<div class="bk-rank-row"><span class="bk-rank-icon">${m.i}</span><span class="bk-rank-lbl">${m.l}</span><span class="bk-rank-name">${r[m.k]}</span></div>`).join('');
  if(medals.every(m=>r[m.k])){
    const fa=document.getElementById('finalsArea');
    if(fa){
      fa.style.display='';
      fa.innerHTML=`<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;">
...
function updateAddFormUI() {
  ...
  const isCX     = isBlade && addForm.series === 'CX';
  ...
  if (isBlade) {
    // BX / UX：下拉選現有戰刃
    if (nameLabel) nameLabel.textContent = '③ 選擇 / 新增鋼鐵戰刃';
    renderBladeSelect(addForm.series, nameWrap);
  } else {
    // 固鎖輪盤 / 軸心：下拉選現有，或輸入新增
    if (nameLabel) nameLabel.textContent = addForm.cat==='ratchet' ? '② 選擇 / 新增固鎖輪盤' : '② 選擇 / 新增軸心';
    renderPartSelect(addForm.cat, nameWrap);
  }
}

// ── 渲染 BX/UX 鋼鐵戰刃下拉
function renderBladeSelect(series, wrap) {
  if (!wrap) return;
  const blades = state.parts.filter(p => p.cat==='blade' && p.series===series);
  wrap.innerHTML = `
    <select class="form-select" id="partDropdown" style="margin-bottom:8px;">
      <option value="__new__">── 新增一把 ${series} 鋼鐵戰刃 ──</option>
      ${blades.map(b=>`<option value="${b.id}">${b.name}${b.spin?' ('+b.spin+')':''}</option>`).join('')}
    </select>
    <div id="partNewInputWrap" style="display:none;margin-top:6px;">
      <input class="form-input" id="partNewInput" placeholder="輸入新鋼鐵戰刃名稱...">
    </div>`;
  document.getElementById('partDropdown').onchange = function() {
    document.getElementById('partNewInputWrap').style.display = this.value==='__new__' ? '' : 'none';
  };
}

// ── 渲染固鎖輪盤 / 軸心下拉
function renderPartSelect(cat, wrap) {
  if (!wrap) return;
  const parts = state.parts.filter(p => p.cat===cat);
  const label = cat==='ratchet' ? '固鎖輪盤' : '軸心';
  wrap.innerHTML = `
    <select class="form-select" id="partDropdown" style="margin-bottom:8px;">
      <option value="__new__">── 新增${label} ──</option>
      ${parts.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}
    </select>
    <div id="partNewInputWrap" style="display:none;margin-top:6px;">
      <input class="form-input" id="partNewInput" placeholder="輸入名稱...">
    </div>`;
  document.getElementById('partDropdown').onchange = function() {
    document.getElementById('partNewInputWrap').style.display = this.value==='__new__' ? '' : 'none';
  };
...
function updateCxPreview() {
  ...
  if (name) {
    preview.innerHTML = `
      <div style="font-size:.7rem;color:var(--text2);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px;">組合名稱預覽</div>
      <div style="font-family:'Rajdhani',sans-serif;font-size:1.2rem;font-weight:700;color:var(--gold-dk);">${name}</div>`;
  } else {
    preview.innerHTML = `<div style="color:var(--text2);font-size:.8rem;">請至少選擇聖獸紋章</div>`;
  }
}

function buildCxName() {
  const getN = id => { if(!id) return ''; const m=state.cxMods.find(m=>m.id==id); return m?m.name:''; };
  const e  = getN(document.getElementById('cxEmblem')?.value);
  const mb = getN(document.getElementById('cxMainBlade')?.value);
  const sb = getN(document.getElementById('cxSubBlade')?.value);
  const ob = getN(document.getElementById('cxOverBlade')?.value);
  if (!e) return '';
  return [e,mb,sb,ob].filter(Boolean).join('');
}
...
function addCxMod() {
  ...
  renderCxModList(); refreshCxSelects();
...
function refreshCxSelects() {
  [['cxEmblem','emblem'],['cxMainBlade','mainBlade'],['cxSubBlade','subBlade'],['cxOverBlade','overBlade']]
    .forEach(([selId,typeKey]) => {
      const el = document.getElementById(selId); if (!el) return;
      const cur = el.value;
      el.innerHTML = `<option value="">— 未使用 —</option>` +
        state.cxMods.filter(m=>m.type===typeKey)
          .map(m=>`<option value="${m.id}"${m.id==cur?' selected':''}>${m.name}</option>`).join('');
    });
  updateCxPreview();
}
...
function addPart() {
  ...
  if (isCX) {
    const name = buildCxName();
    if (!name) { showToast('請至少選擇聖獸紋章','error'); return; }
    if (state.parts.find(p=>p.cat==='blade'&&p.series==='CX'&&p.name===name)) {
      showToast(`「${name}」已存在`,'error'); return;
    }
    const cxMods = {
    ...
    save();
    showToast(`✅ CX 鋼鐵戰刃「${name}」已新增！`,'success');
  ...
  // BX/UX 戰刃 / 固鎖輪盤 / 軸心：共用下拉邏輯
  const dropdown = document.getElementById('partDropdown');
  if (!dropdown) { showToast('請選擇或輸入名稱','error'); return; }

  if (dropdown.value !== '__new__') {
    showToast('此零件已存在零件庫','error'); return;
  }

  const newName = (document.getElementById('partNewInput')?.value||'').trim();
  if (!newName) { showToast('請輸入新零件名稱','error'); return; }

  // 重複檢查
  const dup = state.parts.find(p=>p.cat===addForm.cat&&p.name===newName&&(isBlade?p.series===addForm.series:true));
  if (dup) { showToast(`「${newName}」已存在`,'error'); return; }

  state.parts.push({
    id:Date.now(), name:newName, cat:addForm.cat,
    series: isBlade ? addForm.series : null,
    spin:   isBlade ? document.getElementById('newPartSpin').value : null,
    note:   document.getElementById('newPartNote').value,
    cxMods:{}, uses:0,
  });
  save();
  showToast(`✅ ${CAT_LABELS[addForm.cat]}「${newName}」已新增！`,'success');
  document.getElementById('partNewInput').value='';
  document.getElementById('newPartNote').value='';
  updateAddFormUI();  // 重刷下拉（含新增的零件）
  renderParts();
}
...
function renderParts() {
  ...
  grid.innerHTML = filtered.map(p=>{
    ...
    if(p.cat==='blade'&&p.series==='CX'&&p.cxMods){
      const mods=Object.entries(p.cxMods).filter(([,v])=>v)
        .map(([k,id])=>{ const m=state.cxMods.find(m=>m.id==id); return m?`${CX_MOD_LABELS[k]}：${m.name}`:null; }).filter(Boolean);
...
function showPart(id) {
  ...
  let cx='';
  if(p.cat==='blade'&&p.series==='CX'&&p.cxMods){
    const rows=Object.entries(CX_MOD_LABELS).map(([k,label])=>{
      const mid=p.cxMods[k]; const m=mid?state.cxMods.find(m=>m.id==mid):null;
      return `<div class="part-detail-item"><div class="form-label">${label}</div><div>${m?m.name:'未使用'}</div></div>`;
    });
...
function renderPartStats() {
  ...
  document.getElementById('partUsageChart').innerHTML=sorted.length
    ?sorted.map(p=>`<div class="bar-row"><div class="bar-label">${p.name}</div><div class="bar-track"><div class="bar-fill" style="width:${(p.uses||0)/maxU*100}%"></div></div><div class="bar-val">${p.uses||0}</div></div>`).join('')
    :'<div style="color:var(--text2);">暫無數據</div>';
}
...
function buildMCUI(player) {
  ...
  const ras=state.parts.filter(p=>p.cat==='ratchet');
  const bis=state.parts.filter(p=>p.cat==='bit');

  const bladeOpts=`<option value="">— 未使用 —</option>
    ${bxB.length?`<optgroup label="BX">${bxB.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}</optgroup>`:''}
    ${uxB.length?`<optgroup label="UX">${uxB.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}</optgroup>`:''}
    ${cxB.length?`<optgroup label="CX">${cxB.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}</optgroup>`:''}`;
  ...
  const mkRow=(cat,label,opts)=>{
    if(cat!=='blade'){
      ...
      opts=`<option value="">— 未使用 —</option>${parts.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}`;
    }
    return `<div class="config-step"><div class="config-step-label">${label}</div>
      <select class="form-select" style="padding:7px 12px;font-size:.82rem;" onchange="matchConfigs['${player}']['${cat}']=this.value">${opts}</select></div>`;
  };
  el.innerHTML=mkRow('blade','鋼鐵戰刃',bladeOpts)+mkRow('ratchet','固鎖輪盤','')+mkRow('bit','軸心','');
}
...
function renderPLogConfig() {
  ...
  const cxB=state.parts.filter(p=>p.cat==='blade'&&p.series==='CX');
  const bladeOpts=`<option value="">— 未使用 —</option>
    ${bxB.length?`<optgroup label="BX">${bxB.map(p=>`<option value="${p.id}"${pLogConfig.blade==p.id?' selected':''}>${p.name}</option>`).join('')}</optgroup>`:''}
    ${uxB.length?`<optgroup label="UX">${uxB.map(p=>`<option value="${p.id}"${pLogConfig.blade==p.id?' selected':''}>${p.name}</option>`).join('')}</optgroup>`:''}
    ${cxB.length?`<optgroup label="CX">${cxB.map(p=>`<option value="${p.id}"${pLogConfig.blade==p.id?' selected':''}>${p.name}</option>`).join('')}</optgroup>`:''}`;
  const mkSel=(cat,label,opts)=>{
    if(cat!=='blade'){
      const parts=state.parts.filter(p=>p.cat===cat);
      if(!parts.length) return `<div class="config-step"><div class="config-step-label">${label}</div><div class="no-parts-msg">尚無零件</div></div>`;
      opts=`<option value="">— 未使用 —</option>${parts.map(p=>`<option value="${p.id}"${pLogConfig[cat]==p.id?' selected':''}>${p.name}</option>`).join('')}`;
    }
    return `<div class="config-step"><div class="config-step-label">${label}</div>
      <select class="form-select" style="padding:7px 12px;font-size:.82rem;" onchange="pLogConfig['${cat}']=this.value">${opts}</select></div>`;
  };
  el.innerHTML=mkSel('blade','鋼鐵戰刃',bladeOpts)+mkSel('ratchet','固鎖輪盤','')+mkSel('bit','軸心','');
}
...
function renderPersonalLog() {
  ...
  const cfgText=config=>{
    if(!config) return '';
    const items=['blade','ratchet','bit'].map(cat=>{const pid=config[cat];if(!pid) return null;const p=state.parts.find(p=>p.id==pid);if(!p) return null;return `<span><strong>${CAT_LABELS[cat]}：</strong>${p.name}</span>`;}).filter(Boolean);
